...
public class VerticalOrderTraversalBinaryTree {
  ...
  private void visit(TreeNode root, int col, Map<Integer, List<Integer>> values) {
    if (root == null) {
      return;
    }
    List<Integer> list = values.computeIfAbsent(col, k -> new ArrayList<>());
    insert(list, root.val);

    visit(root.left, col - 1, values);
    visit(root.right, col + 1, values);
  }
