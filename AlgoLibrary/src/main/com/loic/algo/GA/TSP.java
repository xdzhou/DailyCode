...
public class TSP
{
	...
	public void start(int[] x, int[] y)
	{
		...
		for (int i = 0; i < num_generation; i++)
		{
			...
			for (int j = 0; j < chirdrenGene.size() - 1; j++)
			{
				...
				calculeFitness(gene, x, y);
				sommeFitness += gene.getFitness();
				if (gene.getFitness() > chirdrenGene.get(maxFitnessIndi)
						.getFitness())
					maxFitnessIndi = j;
			}
			if (chirdrenGene.get(chirdrenGene.size() - 1).getFitness() > chirdrenGene
					.get(maxFitnessIndi).getFitness())
				maxFitnessIndi = chirdrenGene.size() - 1;
			gene_list.clear();
			gene_list.addAll(chirdrenGene);
		}
		System.out.println("length = "
				+ (MAX_FITNESS - gene_list.get(maxFitnessIndi).getFitness()));
	 ...
	 */
	private void generateChildren(GeneTSP pg1, GeneTSP pg2,
			List<GeneTSP> chirdrenGene)
	{
		Random r = new Random();
	...
	private void calculeFitness(Gene g, int[] x, int[] y)
	{
		...
		{
			f += distancePoints(x[g.chromosome[i]], y[g.chromosome[i]],
					x[g.chromosome[(i + 1) % gene_len]], y[g.chromosome[(i + 1)
							% gene_len]]);
		}
